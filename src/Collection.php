...
abstract class Collection extends \MongoDB\Collection
{
	final public static function getInstance()
	{
		...
		if(!Registry::get($className))
		{
			...
			{
				throw new \Exception("{$className}::getInstance() não localizou 'mongoManager' no Registry.");
			}

			// Obtém o 2º argumento do construtor
			if(!$mongoConfig = Registry::get('mongoConfig'))
			{
				throw new \Exception("{$className}::getInstance() não localizou 'mongoConfig' no Registry.");
			}

			// Obtém o 3º argumento do construtor
			if(!defined("{$className}::COLLECTION_NAME"))
			{
				throw new \Exception("A constante {$className}::COLLECTION_NAME precisa ser definida.");
			}

			// Obtém o 4º argumento do construtor
			if(!defined("{$className}::DOCUMENT_CLASS"))
			{
				throw new \Exception("A constante {$className}::DOCUMENT_CLASS precisa ser definida.");
			}
	...
	final public function save(Document $document)
	{
		...
		{
			throw new \Exception("{$className}::save() só pode ser usado com um objeto do tipo {$documentClass}.");
		}
	...
	final public function delete(Document $document)
	{
		...
		if(!$document instanceof $documentClass)
		{
			throw new \Exception("{$className}::delete() só pode ser usado com um objeto do tipo {$documentClass}.");
		}
