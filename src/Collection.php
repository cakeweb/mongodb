...
abstract class Collection extends \MongoDB\Collection
{
    ...
    final public static function getInstance()
    {
        ...
        {
            $connectionName = $className::CONNECTION_NAME;
            $hint = "Utilize o método \\CakeWeb\\MongoDB\\Connection::init('127.0.0.1:27017', 'data-db', 'user', 'pass', 'auth-db', '{$connectionName}'); para iniciar a conexão com o MongoDB.";

            // Obtém o 1º argumento do construtor
            $mongoManagerName = "mongoManager{$connectionName}";
            if(!$mongoManager = Registry::get($mongoManagerName))
            {
                throw new Exception("{$className}::getInstance() não localizou '{$mongoManagerName}' no Registry. {$hint}", 'CAKE-MONGO-UNINITIALIZED');
            }

            // Obtém o 2º argumento do construtor
            $mongoConfigName = "mongoConfig{$connectionName}";
            if(!$mongoConfig = Registry::get($mongoConfigName))
            {
                throw new Exception("{$className}::getInstance() não localizou '{$mongoConfigName}' no Registry. {$hint}", 'CAKE-MONGO-UNINITIALIZED');
            }
